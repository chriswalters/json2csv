...
function checkParams(params) {
  ...
  if (!Array.isArray(params.data)) params.data = [params.data];
  if (params.flatten) params.data = params.data.map(flatten);
  ...
  createDataRows(params.data, params.unwindPath).forEach(function (dataElement) {
    ...
    if (dataElement && (Object.getOwnPropertyNames(dataElement).length > 0 || params.includeEmptyRows)) {
      ...
      params.fields.forEach(function (fieldElement) {
        ...
        if (val === null || val === undefined) val = defaultValue;
        ...
        if (val !== undefined) {
          ...
          if (stringify || val.toString().includes(params.del) || val.toString().includes('{')) {
            stringifiedElement = JSON.stringify(val).replace(/\\\\/g, '\\');
            if (stringifiedElement.includes('{') && stringifiedElement.includes('}')) {
              stringifiedElement = stringifiedElement.replace(/",\\"/g,'";\\"').replace(/{|\\"|}/g,'');
              stringifiedElement = stringifiedElement.replace(/null,/g,'null;');
              if (stringifiedElement.includes(params.del)) {
          ...
          if (stringifiedElement.toString().startsWith('""') && stringifiedElement.toString().endsWith('""')) {
            stringifiedElement = stringifiedElement.toString().substring(1, stringifiedElement.toString().length - 1);
          }
